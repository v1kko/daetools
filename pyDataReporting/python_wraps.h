...
public:
    ...
    void AddDataReporter(boost::python::object data_reporter)
    {
        boost::python::extract<daeDataReporter_t*> get_datareporter(data_reporter);
        if(!get_datareporter.check())
                daeDeclareAndThrowException(exNotImplemented);

        daeDelegateDataReporter::AddDataReporter(get_datareporter());

        m_pyarrDataReporters.push_back(data_reporter);
    }

    boost::python::list GetDataReporters()
    {
        return getListFromVector(m_pyarrDataReporters); // it was m_ptrarrDataReporters before
    }
...
protected:
    std::vector<boost::python::object> m_pyarrDataReporters;
};
