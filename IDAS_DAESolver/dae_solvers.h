...
//		m_vectorInvMaxElements			= NULL;
    ...
    void DestroySerialArrays(void)
    {
        ...
        if(m_vectorValueConstraints)
            N_VDestroy_Serial(m_vectorValueConstraints);
...
//			DestroyMat(m_matKrylov);
    ...
    void CreateSerialArrays(size_t N)
    {
        ...
        m_vectorValueConstraints = N_VNew_Serial(N);
        if(!m_vectorValueConstraints)
        {
            daeDeclareException(exMiscellanous);
            e << "Unable to allocate vectorValueConstraints array";
            throw e;
        }
        ...
        N_VConst(0, m_vectorValueConstraints);
    }
...
public:
    ...
    N_Vector				m_vectorValueConstraints;
