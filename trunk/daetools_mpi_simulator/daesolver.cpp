...
int solInitialize(daeIDASolver_t* s, void* model, void* simulation, daeLASolver_t* lasolver,
                  long Nequations, long Nequations_local,
                  ...
                  const real_t*  absTolerances, const int* IDs, real_t  relativeTolerance)
{
    ...
    if(lasolver)
    {
        retval = laInitialize(lasolver, pmodel, pmodel->Nequations_local);
        if(check_flag(&retval, "laInitialize", 1))
            return(-1);
...
int solSolveInitial(daeIDASolver_t* s)
{
    ...
    for(iCounter = 0; iCounter < 100; iCounter++)
    {
        if(model->quasiSteadyState)
            retval = IDACalcIC(s->mem, IDA_Y_INIT, 1e-5);
        else
            retval = IDACalcIC(s->mem, IDA_YA_YDP_INIT, 1e-5);
...
int solReinitialize(daeIDASolver_t* s, bool bCopyDataFromBlock, bool bResetSensitivities)
{
    ...
    for(iCounter = 0; iCounter < 100; iCounter++)
    {
         ...
         */
        retval = IDACalcIC(s->mem, IDA_YA_YDP_INIT, s->m_dCurrentTime + 1e-5);
        if(retval < 0)
        {
                 ...
                 void *user_data)
{
    ...
    mpiSynchroniseData(model, yval, ypval);
                         ...
                         N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
    ...
    daeIDASolver_t* dae_solver = (daeIDASolver_t*)user_data;
    daeModel_t*     model      = (daeModel_t*)dae_solver->model;
    daeLASolver_t*  lasolver   = dae_solver->lasolver;

    return laSetup(lasolver, tt, cj, yval, ypval, res);
}
                         ...
                         N_Vector tmp)
{
    ...
    return laSolve(lasolver, tt, c_j, delta, yval, ypval, res, r, z);
}
