...
{
public:
	base_unit(double multi = 0.0, double l = 0.0, double m = 0.0, double t = 0.0, 
	          double c = 0.0, double i = 0.0, double o = 0.0, double n = 0.0)
	{
        L          = l; //length, m
        M          = m; //mass, kg
        T          = t; //time, s
        C          = c; //luminous intensity, cd
        I          = i; //el. current, A
        O          = o; //temperature, K
        N          = n; //amount of a substance, mol
        multiplier = multi;	
	}
	...
	base_unit operator^(const base_unit& other) const
	{
	// Valid only if 'other' is dimensionless (to satisfy a compiler)
		if(other.L != 0 || other.M != 0 || other.T != 0 || other.C != 0 ||
		   other.I != 0 || other.O != 0 || other.N != 0)
		{
...
public:
	...
	std::string toString(void) const
	{
		...
		std::string _units = boost::algorithm::join(arrUnits, __string_unit_delimiter__);
		return (boost::format("%1%") % _units).str();
	}
...
public:
	...
	{
        if(_units != referrer.getUnits())
            throw std::runtime_error((boost::format("Units not consistent: scale from %1% to %2%") % _units.toString() % referrer.getUnits().toString()).str());
	...
	quantity operator+(const quantity& other) const
	{
        if(_units != other.getUnits())
            throw std::runtime_error((boost::format("Units not consistent: %1% + %2%") % _units.toString() % other.getUnits().toString()).str());
		
		quantity tmp(_value + other.getValue(), _units);
		return tmp;	
	}
