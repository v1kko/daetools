...
def consoleRun():
    ...
    for i,eei in enumerate(eeis):
        if i in range(800, 810):
            ...
            ng.SaveGraph('deal.ii_9[%04d].svg' % i)
    sys.exit()
    """
    
    init_file = os.path.join(os.path.realpath(os.path.dirname(__file__)), 'tutorial_dealii_9.init')
    ...
    simulation.StoreInitializationValues("tutorial_dealii_9.init")
