...
adouble create_adouble_from_object(boost::python::object value)
{
    ...
    else
    {
        ...
        e << "Invalid argument type (must be float or double";
        throw e;
        return adouble();
    }
}
...
{
    boost::python::extract<boost::python::list> lValue(value);
    boost::python::extract<adouble_array>       adarrValue(value);

    if(lValue.check())
        return adarr_FromList(lValue());
    else if(adarrValue.check())
        return adarrValue();
    else
    {
        daeDeclareException(exInvalidCall);
        e << "Invalid argument type (must be python list or adouble_array";
        throw e;
                   ...
                   boost::python::dict options)
{
    ...
    return d(a, domain, method, mapOptions);
}
                                ...
                                boost::python::dict      options)
{
    ...
    return d2_array(adarr, domain, method, mapOptions);
}
                                   ...
                                   daeeThermoPackageBasis basis)
{
    ...
    for(boost::python::ssize_t i = 0; i < n; i++)
    {
        double val = extract<real_t>(x1[i]);
        X1.push_back(val);
    }

    boost::python::ssize_t n2 = boost::python::len(x2);
    for(boost::python::ssize_t i = 0; i < n2; i++)
    {
        double val = extract<real_t>(x2[i]);
        X2.push_back(val);
    }
                                   ...
                                   daeeThermoPackageBasis basis)
{
    ...
    adouble_array x1_ = create_adouble_array_from_object(x1);
    adouble       T2_ = create_adouble_from_object(T1);
