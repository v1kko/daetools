...
namespace cs
{
csComputeStackEvaluator_OpenCL::csComputeStackEvaluator_OpenCL(int platformID, int deviceID, std::string buildProgramOptions)
{
    ...
    kernel_equation = clCreateKernel(program, "EvaluateEquation", &ret);
    clCheck(ret);

    kernel_derivative = clCreateKernel(program, "EvaluateDerivative", &ret);
    clCheck(ret);

    kernel_reset_jacobian = clCreateKernel(program, "ResetJacobianData", &ret);
    clCheck(ret);

    kernel_sens_derivative = clCreateKernel(program, "EvaluateSensitivityDerivative", &ret);
    clCheck(ret);
}

csComputeStackEvaluator_OpenCL::~csComputeStackEvaluator_OpenCL()
{
    FreeResources();
}

void csComputeStackEvaluator_OpenCL::FreeResources()
{
    ...
    if(kernel_equation)
        ret = clReleaseKernel(kernel_equation);
    if(kernel_derivative)
        ret = clReleaseKernel(kernel_derivative);
    if(kernel_reset_jacobian)
        ret = clReleaseKernel(kernel_reset_jacobian);
    if(kernel_sens_derivative)
        ret = clReleaseKernel(kernel_sens_derivative);
    if(program)
    ...
    kernel_equation                 = NULL;
    kernel_derivative               = NULL;
    kernel_reset_jacobian           = NULL;
    kernel_sens_derivative          = NULL;
    program                         = NULL;
    command_queue                   = NULL;
    context                         = NULL;
...
void csComputeStackEvaluator_OpenCL::Initialize(bool                     calculateSensitivities,
                                                 ...
                                                 csIncidenceMatrixItem_t* incidenceMatrixItems)
{
    ...
    //printf("numberOfIncidenceMatrixItems      = %d\n", numberOfIncidenceMatrixItems);
    ...
    bufferSize = numberOfIncidenceMatrixItems*sizeof(csIncidenceMatrixItem_t);
    mem_computeStackJacobianItems = clCreateBuffer(context, CL_MEM_READ_ONLY, bufferSize, NULL, &ret);
    clCheck(ret);
    ret = clEnqueueWriteBuffer(command_queue, mem_computeStackJacobianItems, CL_TRUE, 0, bufferSize, incidenceMatrixItems, 0, NULL, NULL);
    clCheck(ret);
    ...
    bufferSize = numberOfIncidenceMatrixItemsToProcess*sizeof(real_t);
    ...
    global_item_size = numberOfIncidenceMatrixItemsToProcess;
...
/* Evaluate equations. */
void csComputeStackEvaluator_OpenCL::EvaluateEquations(csEvaluationContext_t EC,
                                                        real_t*               dofs,
                                                        real_t*               values,
                                                        ...
                                                        real_t*               residuals)
{
...

    {
        ...
        ret = clSetKernelArg(kernel_equation, 0, sizeof(cl_mem),                             (void *)&mem_computeStacks);
        clCheck(ret);
        ret = clSetKernelArg(kernel_equation, 1, sizeof(cl_mem),                             (void *)&mem_activeEquationSetIndexes);
        clCheck(ret);
        ret = clSetKernelArg(kernel_equation, 2, sizeof(csEvaluationContext_t), (void *)&EC);
        clCheck(ret);
        ret = clSetKernelArg(kernel_equation, 3, sizeof(cl_mem),                             (void *)&mem_dofs);
        clCheck(ret);
        ret = clSetKernelArg(kernel_equation, 4, sizeof(cl_mem),                             (void *)&mem_values);
        clCheck(ret);
        ret = clSetKernelArg(kernel_equation, 5, sizeof(cl_mem),                             (void *)&mem_timeDerivatives);
        clCheck(ret);
        ret = clSetKernelArg(kernel_equation, 6, sizeof(cl_mem),                             (void *)&mem_residuals);
        clCheck(ret);
        ...
        ret = clEnqueueNDRangeKernel(command_queue, kernel_equation, 1, NULL, &global_item_size, plocal_item_size, 0, NULL, NULL);
        clCheck(ret);
...
void csComputeStackEvaluator_OpenCL::EvaluateDerivatives(csEvaluationContext_t EC,
                                                         real_t*               dofs,
                                                         real_t*               values,
                                                         real_t*               timeDerivatives,
                                                         real_t*               jacobianItems)
{
    ...
    /* Set OpenCL kernel arguments. */
    ret = clSetKernelArg(kernel_derivative, 0, sizeof(cl_mem),                             (void *)&mem_computeStacks);
    clCheck(ret);
    ret = clSetKernelArg(kernel_derivative, 1, sizeof(cl_mem),                             (void *)&mem_activeEquationSetIndexes);
    clCheck(ret);
    ret = clSetKernelArg(kernel_derivative, 2, sizeof(cl_mem),                             (void *)&mem_computeStackJacobianItems);
    clCheck(ret);
    ret = clSetKernelArg(kernel_derivative, 3, sizeof(csEvaluationContext_t), (void *)&EC);
    clCheck(ret);
    ret = clSetKernelArg(kernel_derivative, 4, sizeof(cl_mem),                             (void *)&mem_dofs);
    clCheck(ret);
    ret = clSetKernelArg(kernel_derivative, 5, sizeof(cl_mem),                             (void *)&mem_values);
    clCheck(ret);
    ret = clSetKernelArg(kernel_derivative, 6, sizeof(cl_mem),                             (void *)&mem_timeDerivatives);
    clCheck(ret);
    ret = clSetKernelArg(kernel_derivative, 7, sizeof(cl_mem),                             (void *)&mem_jacobianItems);
    clCheck(ret);

    /* Execute OpenCL kernel. */
    size_t global_item_size = EC.numberOfIncidenceMatrixItems;
    size_t* plocal_item_size = NULL;

    ret = clEnqueueNDRangeKernel(command_queue, kernel_derivative, 1, NULL, &global_item_size, plocal_item_size, 0, NULL, NULL);
    clCheck(ret);

    /* Copy results from the memory buffer. */
    bufferSize = EC.numberOfIncidenceMatrixItems*sizeof(real_t);
...
void csComputeStackEvaluator_OpenCL::EvaluateSensitivityDerivatives(csEvaluationContext_t EC,
                                                                    real_t*               dofs,
                                                                    real_t*               values,
                                                                    real_t*               timeDerivatives,
                                                                    real_t*               svalues,
                                                                    ...
                                                                    real_t*               sresiduals)
{
    ...
    /* Set OpenCL kernel arguments. */
    ret = clSetKernelArg(kernel_sens_derivative, 0, sizeof(cl_mem),                             (void *)&mem_computeStacks);
    clCheck(ret);
    ret = clSetKernelArg(kernel_sens_derivative, 1, sizeof(cl_mem),                             (void *)&mem_activeEquationSetIndexes);
    clCheck(ret);
    ret = clSetKernelArg(kernel_sens_derivative, 2, sizeof(csEvaluationContext_t), (void *)&EC);
    clCheck(ret);
    ret = clSetKernelArg(kernel_sens_derivative, 3, sizeof(cl_mem),                             (void *)&mem_dofs);
    clCheck(ret);
    ret = clSetKernelArg(kernel_sens_derivative, 4, sizeof(cl_mem),                             (void *)&mem_values);
    clCheck(ret);
    ret = clSetKernelArg(kernel_sens_derivative, 5, sizeof(cl_mem),                             (void *)&mem_timeDerivatives);
    clCheck(ret);
    ret = clSetKernelArg(kernel_sens_derivative, 6, sizeof(cl_mem),                             (void *)&mem_sdvalues);
    clCheck(ret);
    ret = clSetKernelArg(kernel_sens_derivative, 7, sizeof(cl_mem),                             (void *)&mem_sdvalues);
    clCheck(ret);
    ret = clSetKernelArg(kernel_sens_derivative, 8, sizeof(cl_mem),                             (void *)&mem_sresiduals);
    clCheck(ret);
    ...
    ret = clEnqueueNDRangeKernel(command_queue, kernel_sens_derivative, 1, NULL, &global_item_size, plocal_item_size, 0, NULL, NULL);
    clCheck(ret);
