                               ...
                               __global real_t*                   equations)
{
    ...
    /* printf("residual[%d] = %.12f\n", residualIndex, res_cs); */

    /* Set the value in the residuals array. */
    equations[residualIndex] = res_cs;
}

/* Jacobian kernel function. */
void __kernel EvaluateDerivative(__global csComputeStackItem_t*     computeStacks,
                                 __global uint32_t*                 activeEquationSetIndexes,
                                 __global csIncidenceMatrixItem_t*  incidenceMatrixItems,
                                 csEvaluationContext_t              EC,
                                 __global real_t*                   dofs,
                                 __global real_t*                   values,
                                 __global real_t*                   timeDerivatives,
                                 __global real_t*                   derivatives)
{
    int jacobianIndex = get_global_id(0);
    int jacobianItemsIndex = EC.startJacobianIndex + get_global_id(0);

    /* Locate the current equation stack in the array of all compute stacks. */
    csIncidenceMatrixItem_t jacobianItem        = incidenceMatrixItems[jacobianItemsIndex];
    uint32_t firstIndex                         = activeEquationSetIndexes[jacobianItem.equationIndex];
    __global csComputeStackItem_t* computeStack = &computeStacks[firstIndex];

    /* Set the overall index for derivative evaluation. */
    ...
    /* printf("derivatives[%d] ei(%d) bi(%d) fi(%d) = %.12f\n",
               jacobianItemIndex, jacobianItem.equationIndex, jacobianItem.blockIndex, firstIndex, jac_cs.m_dDeriv); */

    /* Set the value in the derivatives array. */
    derivatives[jacobianIndex] = jac_cs.m_dDeriv;
}

/* Evaluate sensitivity derivatives. */
void __kernel EvaluateSensitivityDerivative(__global csComputeStackItem_t*     computeStacks,
                                            __global uint32_t*                 activeEquationSetIndexes,
                                            csEvaluationContext_t              EC,
                                            __global real_t*                   dofs,
                                            __global real_t*                   values,
                                            ...
                                            __global real_t*                   sderivatives)
{
    ...
    sderivatives[residualIndex] = res_cs.m_dDeriv;
}
