...
public:
    ...
    modTutorial1(std::string strName, daeModel* pParent = NULL, std::string strDescription = "")
...
public:
    simTutorial1(void) : model("cdaeTutorial1")
    {
        SetModel(&model);
    }
...
public:
    ...
    */
        model.x.CreateStructuredGrid(25, 0, 0.1);
        model.y.CreateStructuredGrid(25, 0, 0.1);

    // Parameters' value can be set by using a function SetValue.
        model.k.SetValue(401 * W/(m*K));
        model.c_p.SetValue(385 * J/(kg*K));
        model.rho.SetValue(8960 * kg/(m^3));
        model.Q_b.SetValue(1e6 * W/(m^2));
        model.Q_t.SetValue(0 * W/(m^2));
    }
...
{
    std::unique_ptr<daeSimulation_t>    pSimulation  (new simTutorial1);
    ...
    pDataReporter->Connect("", "cdae_tutorial1-" + daetools::getFormattedDateTime());
