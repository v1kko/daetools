...
bool daeTCPIPLog::Connect(const string& strIPAddress, int nPort)
{
    ...
    m_ptcpipSocket = std::shared_ptr<tcp::socket>(new tcp::socket(m_ioService));
...
void daeTCPIPLogServer::Start(void)
{
    ...
    m_pThread = std::shared_ptr<boost::thread>(new boost::thread(boost::bind(&daeTCPIPLogServer::thread, this)));
}
