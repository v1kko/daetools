...
*/
    ...
    // Support for CDATA
    if(nLength > 0 && inString.find("<![CDATA[") != std::string::npos && inString.find("]]>") != std::string::npos)
    {
        aResult = inString;
        return aResult;
    }
...
{
    m_strValue = toStringFormatted<float>(Value, -1, FLT_DIG, false, true);
    LTrim(m_strValue, ' ');
}
void xmlTag::SetValue(double Value)
{
    m_strValue = toStringFormatted<double>(Value, -1, DBL_DIG, false, true);
    LTrim(m_strValue, ' ');
}
