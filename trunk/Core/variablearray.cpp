...
size_t daeDomainIndex::GetHash() const
{
    size_t seed = 0;
    boost::hash_combine(seed, (int)m_eType);
    boost::hash_combine(seed, m_nIndex);
    boost::hash_combine(seed, (std::intptr_t)m_pDEDI);
    boost::hash_combine(seed, m_iIncrement);
    boost::hash_combine(seed, (std::intptr_t)m_pLastPointInDomain);
    return seed;
}
