#ifndef DAE_DOCSTRINGS_H
#define DAE_DOCSTRINGS_H

const char* DOCSTR_base_unit = "";
    const char* DOCSTR_base_unit_L = "";
    const char* DOCSTR_base_unit_M = "";
    const char* DOCSTR_base_unit_T = "";
    const char* DOCSTR_base_unit_C = "";
    const char* DOCSTR_base_unit_I = "";
    const char* DOCSTR_base_unit_O = "";
    const char* DOCSTR_base_unit_N = "";
    const char* DOCSTR_base_unit_multiplier = "";

const char* DOCSTR_unit = "";
    const char* DOCSTR_unit_init = "";
    const char* DOCSTR_unit_baseUnit = "";
    const char* DOCSTR_unit_unitDictionary = "";

const char* DOCSTR_quantity = "";
    const char* DOCSTR_quantity_init = "";
    const char* DOCSTR_quantity_valueInSIUnits = "";
    const char* DOCSTR_quantity_value = "";
    const char* DOCSTR_quantity_units = "";
    const char* DOCSTR_quantity_scaleTo = "";
#endif
