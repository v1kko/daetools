...
void daeObject::Save(io::xmlTag_t* pTag) const
{
    ...
    strName = "Name";
    pTag->Save(strName, "<![CDATA[" + m_strShortName + "]]>");

    strName = "Description";
    pTag->Save(strName, "<![CDATA[" + m_strDescription + "]]>");
...
{
    string::size_type iFound = strChild.find(strParent);
    if(iFound == string::npos || iFound != 0)
        return strChild;
...
//string daeGetRelativeNameAndPrepend(const daeObject* parent, const daeObject* child, const std::string& prependToName)
//{
//    std::string strParent   = (parent ? parent->GetCanonicalName() : "");
//    std::string strVariable = (child ? child->GetCanonicalNameAndPrepend(prependToName) : "");
//    return daeGetRelativeName(strParent, strVariable);
//}
