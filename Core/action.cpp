...
void daeAction::Save(io::xmlTag_t* pTag) const
{
    ...
    if(m_eActionType == eChangeState)
    {
        ...
        strName = "StateTo";
        pTag->SaveObjectRef(strName, m_pStateTo, m_pSTN);
    }
    else if(m_eActionType == eSendEvent)
    {
        ...
        strValue = "$" + m_pSetupNode->SaveAsLatex(&c) + "$";
...
*/
    ...
    {
        strName = "Variable";
        strValue = daeGetRelativeName(m_pModel->GetCanonicalName(), m_pVariableWrapper->GetVariableCanonicalNameWithDomains());
...
void daeAction::SaveRuntime(io::xmlTag_t* pTag) const
{
    ...
    else if(m_eActionType == eSendEvent)
    {
        ...
        // Save it as inline math (use $)!
        daeNodeSaveAsContext c(m_pModel);
        strValue = "$" + m_pNode->SaveAsLatex(&c) + "$";
        pChildTag->SetValue(strValue);
/*
        strName = "Expression";
        adNode::SaveNode(pTag, strName, m_pSetupNode.get());
